        ...
        <article class="blog-post">
          ...
          <div class="blog-post__about">
            <h3 class="blog-post__title">
              <a href="post/<?=$post->getId()?>" title="<?=$post->getTitle()?>"><?=$post->getTitle()?></a>
            </h3>
            <p class="blog-post__text"><?=trim(strip_tags($post->getExcerpt()))?>
            </p>
            <div class="blog-post__info">
              <img class="blog-post__author-img" src="<?=$post->getUserAvatar()?>" width="40" height="40" alt="<?=$post->getUserName()?>">
              <div class="blog-post__author">
                <p class="blog-post__author-name"><?=$post->getUserName()?></p>
                <p class="blog-post__author-position"><?=$post->getUserNiceName()?></p>
              </div>
                <a href="post/<?=$post->getId()?>"><button type="button" class="btn btn_type_secondary_white blog-post__btn">Читать дальше</button></a>
            </div>
          </div>
        </article>
        <?php } ?>
